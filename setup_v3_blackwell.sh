#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════
# Gonka V3 Benchmark Setup — vLLM 0.14.0 + gonka_poc (Blackwell)
# For: B300, B200, B100, GB200 (Blackwell architecture, SM 10.x)
...
echo "║     BLACKWELL: B300/B200/B100/GB200 (benchmark only)            ║"
echo "╚══════════════════════════════════════════════════════════════════╝"
echo ""
...
if [ "$DRIVER_MAJOR" -lt "$MIN_DRIVER_MAJOR" ]; then
    log_error "Driver version $DRIVER_VERSION_FULL is too old for Blackwell! Minimum: $MIN_DRIVER_MAJOR"
    exit 1
fi
...
# ═══════════════════════════════════════════════════════════════════════════
# FIX: Triton ptxas for Blackwell (sm_103a) support
# ═══════════════════════════════════════════════════════════════════════════
log_info "Fixing Triton ptxas for Blackwell (sm_103a) support..."
...
if [ -n "$TRITON_PATH" ]; then
    ...
    if [ -f "$SYSTEM_PTXAS" ]; then
        ...
        log_warning "System ptxas not found at $SYSTEM_PTXAS"
    fi
else
    log_warning "Triton not found, skipping ptxas fix"
fi
...
echo ""
echo "╔══════════════════════════════════════════════════════════════════╗"
echo "║     V3 SETUP COMPLETE — BLACKWELL (B300/B200)                    ║"
echo "╠══════════════════════════════════════════════════════════════════╣"
echo "║  vLLM: ${VLLM_VERSION} + gonka_poc + Triton ptxas fix              ║"
