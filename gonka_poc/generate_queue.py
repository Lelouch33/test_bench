...
- V0: Uses engine_client.poc_request("generate_artifacts", {...})
- V1: Uses PoCManagerV1.generate_artifacts(...) directly
"""
import asyncio
import os
import time
...
class GenerateJob:
    ...
    request_id: str
    poc_manager: Any  # PoCManagerV1 instance
    app_id: int
    block_hash: str
    block_height: int
    public_key: str
...
class GenerateQueue:
    ...
    async def ensure_worker_running(self, poc_manager, app_id: int):
        """Ensure the worker task is running."""
        if self._worker_task is None or self._worker_task.done():
            self._stop_event.clear()
            self._worker_task = asyncio.create_task(
                self._worker_loop(poc_manager, app_id)
            )
    ...
    async def _worker_loop(self, poc_manager, app_id: int):
        """Background worker that processes queued jobs."""
        logger.info("Generate queue worker started (V1)")
        ...
        logger.info("Generate queue worker stopped (V1)")
    
    async def _process_job(self, job: GenerateJob) -> Dict[str, Any]:
        """Process a single generate job using PoCManager directly."""
        ...
        for i in range(0, total_nonces, job.batch_size):
            ...
            # V1: Use PoCManager directly instead of engine_client.poc_request
            artifacts = await job.poc_manager.generate_artifacts(
                nonces=chunk,
